...
def web():
    ...
    from nexura.config import WEB_HOST, WEB_PORT
...
def scan(prompt, target, output, output_format, deep):
    ...
    console.print(Panel(Markdown(f"**Maqsad:** {plan.intent}\n\n**Reasoning:** {plan.reasoning}"), title="🎯 Reja", border_style="green"))
    ...
    report.end_time = __import__("datetime").datetime.now()
...
def quick(target, ports):
    ...
    with console.status("[bold green]Portlar tekshirilmoqda...[/]") as status:
...
def list_models():
    ...
    from nexura.config import MODELS_DIR
