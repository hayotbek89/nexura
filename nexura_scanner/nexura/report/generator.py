...
from nexura.models.schemas import ScanReport, ScanResult, Vulnerability, PortInfo
...
class ReportGenerator:
    ...
    def save(self, report: ScanReport, fmt: str = "both", path: Optional[str] = None) -> Optional[str]:
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        safe = report.target.replace("://", "_").replace("/", "_")[:30]
        base = path or str(config.REPORTS_DIR / f"{safe}_{ts}")
